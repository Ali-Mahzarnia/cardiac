...
no_outlr_data = rbind(no_outlr_data, tempp)
  ...
  if (i <21){  lm1 <- lm(unlist(data[,i]) ~ Sex*Age*Diet*HN*Geno3, data)  }
...
write.xlsx2(posthoc2, paste0(outpath,prefix,"cardiac_anova.xlsx"), sheet =paste0(colnames(data)[i],"_HN_emmeans_"),append = TRUE)
...
j=1
for (i in c(8:15,19,20,21)) {
  plot_list[[j]]<-ggplot(data, aes(x=Age, y=unlist(data[,i]), color=Geno3, fill = Geno3)) +#, alpha=Diet
  ...
  panel_border() + 
  theme_bw()+
  labs(x = "Age", y = paste0(colnames(data)[i]), title = paste0(colnames(data)[i]) ) +
  theme(legend.position="bottom")
...
j=1
for (i in c(8:15,19,20,21)) {
  plot_list[[j]]<-ggplot(data, aes(x=diet_num, y=unlist(data[,i]), color=Geno3, fill = Geno3)) +#, alpha=Diet
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3'))+
  geom_point(aes(shape = Geno3)) + 
  facet_grid(. ~ Sex) + 
  ...
  panel_border() + 
  theme_bw()+
  labs(x = "Diet", y = paste0(colnames(data)[i]), title = paste0(colnames(data)[i]) ) +
  theme(legend.position="bottom")

ggsave(paste0(outpath,"DietLines_" ,colnames(data)[i], "042623.pdf"), plot = last_plot(), device='pdf', scale=1, width=mywidth, height=myheight, unit=c("in"), dpi=200)
j=j+1
}
...
no_outlr_data = rbind(no_outlr_data, tempp)
  ...
  plot_list[[j]]<-ggplot(data, aes(x=diet_num, y=unlist(data[,i]), color=Sex, fill = Sex)) +#, alpha=Diet
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3'))+
...
ggsave(paste0(outpath,"TestDietLines_" ,colnames(data)[i], "042623.pdf"), plot = last_plot(), device='pdf', scale=1, width=mywidth, height=myheight, unit=c("in"), dpi=200)
j=j+1
}
...
ggsave(paste0(outpath,"TestSexSimpleDietLines_" ,colnames(data)[i], "042623.pdf"), plot = last_plot(), device='pdf', scale=1, width=mywidth, height=myheight, unit=c("in"), dpi=200)
j=j+1
}
