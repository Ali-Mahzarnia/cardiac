...
for (i in c(8:15,19,20,21)) {
  ...
  plot_list[[(i-7)]]<-ggplot(data, aes(x=Geno3, y=unlist(data[,i]), fill = Diet, alpha=Diet)) +
  geom_violin(inherit.aes=TRUE,position=dodge) +
  scale_color_manual(values=mycolors)+
  scale_fill_manual(values=mycolors)+
  ##facet_grid(. ~ Sex)  +
...
plot_list = vector(mode = "list", length = length(c(8:15,19,20,21)))
for (i in c(8:15,19,20,21)) {
  plot_list[[(i-7)]]<-ggplot(data, aes(x=Age, y=unlist(data[,i]), color=Geno3, fill = Geno3)) +#, alpha=Diet
...
plot_list = vector(mode = "list", length = length(c(8:15,19,20,21)))
for (i in c(8:15,19,20,21)) {
  plot_list[[(i-7)]]<-ggplot(data, aes(x=Mass, y=unlist(data[,i]), color=Geno3, fill = Geno3)) +#, alpha=Diet
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3'))+
  geom_point(aes(shape = Geno3)) + 
  facet_grid(. ~ Sex) + 
  ...
  panel_border() + 
  theme_bw()+
  labs(x = "Mass", y = paste0(colnames(data)[i]), title = paste0(colnames(data)[i]) ) +
  theme(legend.position="bottom")
...
plot_list = vector(mode = "list", length = length(c(8:15,19,20,21)))
for (i in c(8:15,19,20,21)) {
  plot_list[[(i-7)]]<-ggplot(data, aes(x=Mass, y=unlist(data[,i]), color=Sex, fill = Sex)) +#, alpha=Diet
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3'))+
...
plot_list = vector(mode = "list", length = length(c(8:15,19,20,21)))
for (i in c(8:15,19,20,21)) {
  plot_list[[(i-7)]]<-ggplot(data, aes(x=Age, y=unlist(data[,i]), color=Diet, fill = Diet)) +#, alpha=Diet
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red', 'azure3'))+
  geom_point(aes(shape = Geno3)) + 
  facet_grid(. ~ Geno3) + 
